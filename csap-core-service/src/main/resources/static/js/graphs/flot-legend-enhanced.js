...
//        var style = window.getComputedStyle(document.querySelector('body'));
        var style = window.getComputedStyle(options.legend.container);
 		console.log(`switched to options.legend.container xxx`) ;
	...
	console.log("registering legend") ;
	for (var i =  $.plot.plugins.length - 1; i >= 0; --i) {
		console.log(`$.plot.plugins[i].name:  ${$.plot.plugins[i].name}`) ;
	    if ($.plot.plugins[i].name == "legend") {
	        $.plot.plugins.splice(i,1);
	    }
	}
    $.plot.plugins.push({
        init: init,
