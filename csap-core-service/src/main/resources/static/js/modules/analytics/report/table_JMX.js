...
const jmxTable = table_jmx();

export default jmxTable
...
function table_jmx() {
	...
	return {
		...
		//
		buildDetailRow: function ( rowJson, tableRow, rowData ) {
			buildDetailRow( rowJson, tableRow, rowData );
		},
		//
		buildCustomRow: function ( rowJson, tableRow, rowData ) {
			buildCustomRow( rowJson, tableRow, rowData );
		},
		//
	}


	function buildSummaryRow( rowJson1, tableRow, rowJson ) {
	...
	function buildCustomRow( rowJson1, tableRow, rowJson ) {
		...
		if ( _jmxCustomNeedsInit ) {
			...
			$( "#jmxCustomTable" ).tablesorter( {
				...
				widgetOptions: {
					columnSelector_container: $( '#applicationdetailColumnSelector' ),
	...
	function buildCustomerHeaderAndFooter( rowJson, tableId ) {
		...
		if ( $( '#applicationdetailColumnSelector input' ).length > 0 ) {
			$( '#applicationdetailColumnSelector input' ).each( function () {
	...
	function buildDetailRow( rowJson1, tableRow, rowJson ) {

		// console.log( JSON.stringify(rowJson, null,"\t") )

		$( "#serviceLabel" ).html( rowJson.serviceName + " Java Report " );
