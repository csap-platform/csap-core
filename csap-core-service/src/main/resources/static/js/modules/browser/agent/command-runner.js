...
import templates from "./command-template.js"
...
const commandRunner = command_runner();
...
function command_runner() {
    ...
    return {
        show: function ( $menuContent, forceHostRefresh, menuPath ) {
            ...
            if ( !_command_editor || forceHostRefresh ) {
                ...
                templatesPromise = getHostConfiguration();

            } else {
                // leave display alone - editing may have occured
            }
     ...
     * @borrows { browser_utils } page parameters are optionally used to pass a template name from an internal launch
     * @returns { Promise }  fetch result promise
     */
    function getHostConfiguration() {

        let url = utils.getCsapBrowserUrl() + "/host-configuration";

        let templatePath = utils.getPageParameters().get( "fromFolder" );

        _dom.logHead( `loading ${ url } with template: ${ templatePath }` );

        let viewLoadedPromise = _net.httpGet( url, {
            fromFolder: templatePath,
            sampleParam: `sampleValue`
        } );

        viewLoadedPromise
            .then( configurationReport => {
    ...
    function initialize( configurationReport ) {
        ...
        _dom.logInfo( `loading configurationReport`, configurationReport );
