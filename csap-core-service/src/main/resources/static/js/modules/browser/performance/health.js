...
const perfHealth = performance_health();

export default perfHealth


function performance_health() {
    ...
    function getAlerts() {
        ...
        let viewLoadedPromise = _net.httpGet( `${ HEALTH_REPORT_URL }`, paramObject );

        viewLoadedPromise
            .then( alertReport => {

                _dom.logArrow( `alertReport: ${ alertReport.length }` );

                $alertsBody.empty();
                let alerts = alertReport.triggered;
                if ( alerts.length == 0 ) {
                    let $row = jQuery( '<tr/>', {} );
                ...
                $( "span:nth-child(1)", $filterCounts ).text( alertReport.filterTotal );
                $( "span:nth-child(2)", $filterCounts ).text( alertReport.storeTotal );
