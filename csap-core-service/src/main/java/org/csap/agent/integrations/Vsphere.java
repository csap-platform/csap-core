...
public class Vsphere {
	...
	public Vsphere ( ObjectMapper jsonMapper, Application csapApplication ) {
		this.jsonMapper = jsonMapper ;
		this.osManager = csapApplication.getOsManager( ) ;
	...
	public String govcRun ( List<String> command ) {
		...
		File workingDir = csapApplication.getCsapInstallFolder( ) ;

		var hungProcessIntervalInSeconds = 5 ;
		var hungProcessMaxIterations = 5 ;
		BufferedWriter outputWriter = null ;

		if ( ! csapApplication.environmentSettings( ).isVsphereConfigured( ) ) {
			logger.warn( "Missing vsphere configuration" ) ;
			return "Not configured" ;
		}

		var environmentVariables = csapApplication.environmentSettings( ).getVsphereEnv( ) ;
	...
	public ArrayNode dataStores ( boolean filterUsingConfiguration ) {
		...
		List<String> collectionScripts = osManager.getOsCommands( ).getGovcDatastoreList( ) ;

		String scriptOutput = govcRun( collectionScripts ) ;

		logger.debug( "scriptOutput: {}", scriptOutput ) ;

		scriptOutput = csapApplication.check_for_stub( scriptOutput, "govc/datastore-all.txt" ) ;
		...
		if ( filterUsingConfiguration ) {
			var datastoreFilter = csapApplication.environmentSettings( ).getVsphereConfiguration( ).at(
	...
	public ObjectNode dataStore_info ( String datastoreName ) {
		...
		List<String> infoCommand = osManager.getOsCommands( ).getGovcDatastoreInfo( datastoreName ) ;

		String infoOutput = govcRun( infoCommand ) ;

		infoOutput = csapApplication.check_for_stub( infoOutput, "govc/datastore-info.json" ) ;
		...
		try {
			...
			datastore.put( "FreeSpace", CsapCore.getDiskWithUnit( dataStoreFull.at( "/Summary/FreeSpace" )
					.asLong( ) ) ) ;
			datastore.put( "Capacity", CsapCore.getDiskWithUnit( dataStoreFull.at( "/Summary/Capacity" ).asLong( ) ) ) ;

			var listing = datastore.putObject( "datastore files" ) ;
			listing.put( DockerJson.list_folderUrl.json( ), "vsphere/datastore/files" ) ;
			...
			datastore.put( DockerJson.error.json( ), "failed api request" ) ;
			datastore.put( DockerJson.errorReason.json( ), reason ) ;
		}
	...
	public ArrayNode datastore_files ( String datastoreName , String path ) {
		...
		List<String> lsCommand = osManager.getOsCommands( ).getGovcDatastoreLs( datastoreName, path ) ;

		String lsOutput = govcRun( lsCommand ) ;

		lsOutput = csapApplication.check_for_stub( lsOutput, "govc/list-datastore.json" ) ;
		logger.debug( "command: {}, \n\t output: \n {}", lsCommand, lsOutput ) ;
		...
		try {
			...
			error.put( DockerJson.error.json( ), "failed api request" ) ;
			error.put( DockerJson.errorReason.json( ), reason ) ;
	...
	synchronized public ObjectNode datastore_file_add ( String datastoreName , String diskPath , String diskSize ,
			String diskType ) {
		...
		result.put( DockerJson.response_plain_text.json( ), resultMessage ) ;
	...
	private String runFindCommand ( String diskPath ) {
		...
		var csapBashFunctions = csapApplication.getOsManager( ).sourceCommonFunctions( ) ;

		List<String> findCommand = List.of(
				"#!/bin/bash",
				csapBashFunctions,
	...
	public ObjectNode datastore_files_recurse ( String datastoreName ) {
		...
		List<String> lsCommand = osManager.getOsCommands( ).getGovcDatastoreRecurse( datastoreName ) ;

		String lsOutput = govcRun( lsCommand ) ;

		lsOutput = csapApplication.check_for_stub( lsOutput, "govc/list-datastore-recursive.json" ) ;
		logger.debug( "command: {}, \n\t output: \n {}", lsCommand, lsOutput ) ;
		...
		try {
			...
			var reason = CSAP.buildCsapStack( e ) ;
			logger.warn( "Failed reading {} {}", datastoreName, reason ) ;
			fileListing.put( DockerJson.error.json( ), "failed api request" ) ;
			fileListing.put( DockerJson.errorReason.json( ), reason ) ;
		}
	...
	public ArrayNode vm_find ( String vmFilter ) {
		...
		List<String> govcFindVms = osManager.getOsCommands( ).getGovcVmFind( vmFilter ) ;
		...
		scriptOutput = csapApplication.check_for_stub( scriptOutput, "govc/vm-find.txt" ) ;
		String[] serviceLines = scriptOutput.split( OsManager.LINE_SEPARATOR ) ;
	...
	public ArrayNode vm_list ( String path ) {
		...
		List<String> govcListVmFolders = osManager.getOsCommands( ).getGovcVmList( path ) ;
		...
		govcListOutput = csapApplication.check_for_stub( govcListOutput, "govc/vm-list.txt" ) ;
	...
	public ObjectNode vmInfo ( String vmPath ) {
		...
		List<String> vmInfoCommand = osManager.getOsCommands( ).getGovcVmInfo( vmPath ) ;

		String vmInfoOutput = govcRun( vmInfoCommand ) ;

		logger.debug( "vmInfoCommand: {}, \n\t vmInfoOutput: \n {}", vmInfoCommand, vmInfoOutput ) ;

		vmInfoOutput = csapApplication.check_for_stub( vmInfoOutput, "govc/vm-info.json" ) ;
		...
		try {
			...
			if ( diskInfo.isArray( ) ) {
				...
				CSAP.jsonStream( diskInfo ).forEach( diskDetails -> {
					...
					var sizeWithUnits = CsapCore.getDiskWithUnit( size ) ;
			...
			vm.put( DockerJson.error.json( ), "failed api request" ) ;
			vm.put( DockerJson.errorReason.json( ), reason ) ;
