...
public class HealthManager {
	...
	private void buildErrorReportsForAdmin ( ObjectNode summaryReport , ObjectNode detailReport ,
			double alertLevel , boolean includeKubernetesWarnings ,
	...
	private void alertsForServices ( String hostName , String clusterFilter , ObjectNode serviceHealthCollected ,
			ArrayNode summaryErrors , double alertLevel ,
			boolean includeKubernetesWarnings ,
	...
	private void alertsForHostCpu ( ObjectNode hostStatusResponse ,
			String hostName ,
			double alertLevel ,
	...
	public ObjectNode build_host_summary_report ( String projectName ) {
		...
		if ( application.isAgentProfile( ) ) {
			...
			if ( application.isKubernetesInstalledAndActive( ) ) {
				...
				volumeCount = (int) application.getKubernetesIntegration( ).listingsBuilder( ).persistentVolumeCount( null )  ;
			...
			for ( var host : environmentHostNames ) {
				...
				if ( agent_status != null ) {
					...
					var hostPodRestartCount = hostStatus.path( "kubernetes" ).path( "podReport" ).path( "restarts" ).asInt( ) ;
