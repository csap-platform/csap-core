...
public class OsSharedResourcesCollector extends HostCollector implements Runnable {
	...
	private void checkForLowMemoryEventGeneration ( int nextIndex ) {
		...
		if ( collectionIntervalSeconds == csapApplication
			...
			logger.warn( "Low memory detected on VM, refer to events log: " + memoryAvailableLessCache[nextIndex] + " MB remaining." ) ;
	...
	private ObjectNode buildAttributeReport ( int requestedSampleSize ,
			int skipFirstItems ) {
