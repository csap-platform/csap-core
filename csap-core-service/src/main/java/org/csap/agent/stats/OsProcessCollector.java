...
public class OsProcessCollector extends HostCollector implements Runnable {
	...
	private void add_latest_statistics_to_cache ( ) {
		...
		ObjectNode os_metrics = osManager.buildServiceStatsReportAndUpdateTopCpu( true ) ;

		if ( os_metrics != null && os_metrics.has( "ps" ) ) {
			ObjectNode latestServiceStatistics = (ObjectNode) os_metrics.get( "ps" ) ;
	 ...
	 *
	 */
	private void cleanUpServiceCache ( ObjectNode latestServiceStatistics ) {
		...
		List<String> services_to_remove = service_to_metricsArrays.keySet( ).stream( )
				.filter( serviceNameInCache -> {

					JsonNode serviceStatus = latestServiceStatistics.path( serviceNameInCache ) ;
		...
		if ( services_to_remove.size( ) > 0 ) {
			logger.warn(
					"Did not find the following service(s) in latest collection, removing from collection cache: \n\t {}",
					services_to_remove ) ;
			services_to_remove.stream( )
					.forEach( service_to_metricsArrays::remove ) ;
			services_to_remove.stream( )
					.forEach( kubernetes_services::remove ) ;
		}

		// StreamSupport.stream(service_names.spliterator(), false) ;

		// while (keyIter.hasNext()) {
		// String serviceName = keyIter.next();
		//
		// if ( !latestServiceStatistics.has( serviceName ) ) {
		// keyIter.remove();
	...
	public ObjectNode buildCollectionReport ( boolean isUpdateSummary ,
...
//		servicesFilter.addAll( serviceNames ) ;
	...
	private ObjectNode updateMetricsDataAndbuildSummaryReport (
			int numSamples , int requestedSampleSize ,
			List<String> serviceIds ,
			ObjectNode metricsAttributeSection , ObjectNode metricsDataSection ) {
		...
		for ( var serviceId : serviceIds ) {
			...
			if ( serviceInstance == null ) {
				logger.warn( "Did not locate: {}", serviceId ) ;
				continue ;
			}
			...
			if ( serviceOsMetrics != null ) {
				...
				// add container counts for calculating total - unless - it is in aggregation mode
