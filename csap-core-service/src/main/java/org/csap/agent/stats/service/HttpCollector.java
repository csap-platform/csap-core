...
public class HttpCollector {
	...
	private void processApplicationCollection ( ServiceInstance serviceInstance , String containerId ,
			ServiceCollectionResults applicationResults ,
			ObjectNode httpConfig , String patternMatch , ResponseEntity<String> collectionResponse )
			throws IOException {
	...
	private void processJavaCollection ( boolean isTomcat , String containerId ,
			ServiceCollectionResults applicationResults ,
	...
	private ResponseEntity<String> perform_collection (
		...
		if ( csapApplication.isDesktopProfileActive( ) && httpCollectionUrl.contains( "localhost" ) ) {
			...
			if ( printLocalWarning ) {
				logger.warn( CsapApplication.testHeader( "Switching localhost to csap-dev01: {}" ), serviceInstance.getName( ) ) ;
				printLocalWarning = false ;
			}

			httpCollectionUrl = httpCollectionUrl.replaceAll(
					Matcher.quoteReplacement( "localhost.lab.sensus.net:7011/api" ),
					Matcher.quoteReplacement( "csap-dev01.lab.sensus.net:8011/api" ) ) ;

			httpCollectionUrl = httpCollectionUrl.replaceAll(
					Matcher.quoteReplacement( "localhost.lab.sensus.net" ),
					Matcher.quoteReplacement( "csap-dev01.lab.sensus.net" ) ) ;
			...
			if ( httpCollectionUrl.contains( "server-status" ) ) {
				httpCollectionUrl = "http://csap-dev01.lab.sensus.net:8011/service/httpd?auto" ;
			}
	...
	private void processHttpMeterUsingRegex (
			ServiceMeter serviceMeter , String containerId ,
			String patternMatch , ServiceCollectionResults applicationResults ,
	...
	private void processHttpMeterUsingJson ( String serviceName ,
			ServiceMeter serviceMeter , String containerId , String patternMatch ,
			ServiceCollectionResults applicationResults ,
		...
		try {
			...
			if ( serviceMeter.getHttpAttribute( ).equals( "csapHostCpu" ) ) {
				...
				//var instance = csapApplication.findServiceByNameOnCurrentHost( serviceNameToCount ) ;
//				if ( instance != null ) {
...
//				}
	...
	private double deltaDecimal ( ServiceMeter serviceMeter , String containerId , double collectedValueAsDouble ,
			double divideBy ,
			double multiplyBy ) {
