...
class Verify_Vsphere extends CsapThinTests {
	...
	void verify_govc_commands ( ) {
		...
		assertThat( commandResults )
				.contains( "GOVC_DATACENTER=MOC RNI Lab" ) ;
	}
	...
	void verify_vm_listing ( ) {
		...
		var vms = vsphere.vm_find( "/MOC RNI Lab/vm" ) ;
		...
		assertThat( vms.get( 0 ).asText( ) )
				.isEqualTo( "[govc, find, /MOC RNI Lab/vm, -type, m]" ) ;

		vms = vsphere.vm_find( "/MOC RNI Lab/vm/RNIs/CSAP-DEV_p" ) ;
		logger.debug( "vm: {}", CSAP.jsonPrint( vms ) ) ;
		logger.info( "vm 0 : {}", CSAP.jsonPrint( vms.get( 0 ) ) ) ;
		logger.info( "vm 1 : {}", CSAP.jsonPrint( vms.get( 1 ) ) ) ;
		...
		assertThat( vms.get( 0 ).asText( ) )
				.isEqualTo( "[govc, find, /MOC RNI Lab/vm/RNIs/CSAP-DEV_p, -type, m]" ) ;
	}
	...
	void verify_vm_folder_list ( ) {
		...
		assertThat( vms_folder_list.at( "/0/attributes/path" ).asText( ) )
				.isEqualTo( "[govc, ls, -l=true, /MOC RNI Lab/vm]" ) ;
		...
		assertThat( vms_folder_list.at( "/39/attributes/path" ).asText( ) )
				.isEqualTo( "/MOC RNI Lab/vm/Flex Net Sim/" ) ;

		var vms_folder_path_find = vsphereExplorer.vm_listing( false, "/MOC RNI Lab/vm/SecureCommandServer/" ) ;
	...
	void verify_vm_info ( ) {
		...
		var vm = vsphere.vmInfo( "/MOC RNI Lab/vm/RNIs/CSAP-DEV_p/csap-dev2/csap-dev07" ) ;

		logger.info( "vm: {}", CSAP.jsonPrint( vm ).substring( 0, 400 ) ) ;

		assertThat( vm.at( "/details/command" ).asText( ) )
				.isEqualTo( "[govc, vm.info, -json=true, /MOC RNI Lab/vm/RNIs/CSAP-DEV_p/csap-dev2/csap-dev07]" ) ;
