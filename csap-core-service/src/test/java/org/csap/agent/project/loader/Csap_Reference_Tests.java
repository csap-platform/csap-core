...
class Csap_Reference_Tests extends CsapBareTest {
	...
	class Dev_Lifecycle {
		...
		void beforeAll ( )
			...
			logger.info( CsapApplication.testHeader( ) ) ;
			var hostNameForEnvResolution = "csap-dev01.lab.sensus.net" ;
...
//			
		...
		void verify_plus_one_ports ( ) {
			...
			assertThat( service.getUrl( ) ).as( "url with port" ).contains(
					"http://csap-dev01.lab.sensus.net:6444/stats" ) ;
		...
		void verify_launch_url ( )
			...
			assertThat( firstPostgres.getUrl( ) )
					.isEqualTo(
							"http://csap-dev04.lab.sensus.net:8014/api/v1/namespaces/csap-test/services/test-k8s-postgres-service" ) ;
